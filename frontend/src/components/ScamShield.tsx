...
import {
  ...
  ChevronDown, ChevronUp, Info, IndianRupee, Hash
} from "lucide-react";
...
function RiskBadge({ risk }: { risk: "SAFE" | "SUSPICIOUS" | "HIGH_RISK" }) {
  const config = {
    SAFE:      { label: "SAFE", color: "text-emerald-400", bg: "bg-emerald-500/10 border-emerald-500/20", icon: <CheckCircle className="w-5 h-5" />, glow: "shadow-emerald-500/20" },
    SUSPICIOUS:{ label: "SUSPICIOUS", color: "text-amber-400", bg: "bg-amber-500/10 border-amber-500/20", icon: <AlertTriangle className="w-5 h-5" />, glow: "shadow-amber-500/20" },
    HIGH_RISK: { label: "HIGH RISK", color: "text-red-400", bg: "bg-red-500/10 border-red-500/20", icon: <XCircle className="w-5 h-5" />, glow: "shadow-red-500/30" },
  }[risk];

  return (
    <div className={`inline-flex items-center gap-2.5 px-5 py-2.5 rounded-2xl border ${config.bg} shadow-xl ${config.glow}`}>
      <span className={config.color}>{config.icon}</span>
      <span className={`text-base font-black tracking-widest font-display ${config.color}`}>{config.label}</span>
    </div>
  );
}

function ConfidenceBar({ value, risk }: { value: number; risk: string }) {
  const color = risk === "SAFE" ? "from-emerald-500 to-cyan-500" : risk === "SUSPICIOUS" ? "from-amber-500 to-orange-500" : "from-red-500 to-rose-500";
  return (
    <div className="space-y-1.5">
      <div className="flex justify-between items-center">
        <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest font-display">Confidence</span>
  ...
  return (
    <div className="relative flex flex-col items-center justify-center">
      ...
      <div className="text-center mt-[-12px]">
        ...
        <p className="text-[9px] font-bold text-slate-600 uppercase tracking-widest mt-0.5">Risk Score</p>
      </div>
    </div>
  );
}
...
export default function ScamShield() {
  const [activeTab, setActiveTab] = useState<"message" | "behavior">("message");
  ...
  }, [message, timeOfMessage, amount, timeOfTransaction, frequency, activeTab, isAnalyzing]);
  ...
  // derived props for Message Result
  const allSignals = messageResult ? [
  ...
  const hasSignal = (key: string) => allSignals.some(s => s.includes(key));

  return (
    <div className="flex flex-col h-full overflow-hidden bg-transparent">
      {/* ─── Header ─── */}
      <div className="px-8 pt-6 pb-6 border-b border-white/[0.06] shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="relative">
              <div className="absolute inset-0 bg-violet-500/20 blur-xl animate-pulse" />
              <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-violet-600 to-fuchsia-600 flex items-center justify-center relative z-10 border border-white/20 shadow-xl shadow-violet-500/20">
                <Shield className="w-6 h-6 text-white" />
              </div>
            </div>
            <div className="font-display">
              <h2 className="text-xl font-black text-white uppercase tracking-tight">Scam Shield</h2>
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-0.5">Dual-Mode Engine</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            {analysisCount > 0 && (
              <div className="flex items-center gap-2 px-3 py-1.5 bg-violet-500/10 border border-violet-500/20 rounded-xl font-display">
                <Zap className="w-3.5 h-3.5 text-violet-400" />
                <span className="text-[10px] font-black text-violet-400 uppercase tracking-widest">{analysisCount} scanned</span>
              </div>
            )}
            <div className="flex items-center gap-2 px-3 py-1.5 bg-red-500/10 border border-red-500/20 rounded-xl font-display">
              ...
              <span className="text-[10px] font-black text-red-400 uppercase tracking-widest">High Recall Active</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        <div className="p-8 space-y-6 max-w-4xl mx-auto">
          ...
          {/* ── Tabs ── */}
          <div className="flex bg-white/[0.03] border border-white/[0.06] rounded-2xl p-1 shrink-0">
            <button
              onClick={() => setActiveTab("message")}
              className={`flex-1 py-3 justify-center items-center flex gap-2 rounded-xl text-xs font-black uppercase tracking-widest font-display transition-all ${activeTab === "message" ? "bg-violet-500/20 text-violet-400 shadow-lg border border-violet-500/20" : "text-slate-500 hover:text-slate-300"}`}
            >
              <Send className="w-4 h-4" /> Message Scanner
            </button>
            <button
              onClick={() => setActiveTab("behavior")}
              className={`flex-1 py-3 justify-center items-center flex gap-2 rounded-xl text-xs font-black uppercase tracking-widest font-display transition-all ${activeTab === "behavior" ? "bg-cyan-500/20 text-cyan-400 shadow-lg border border-cyan-500/20" : "text-slate-500 hover:text-slate-300"}`}
            >
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {activeTab === "message" ? (
                <>
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] font-display">Text Context</label>
                  <div className="relative">
                    <textarea
                      value={message}
                      ...
                      className="w-full bg-white/[0.03] border border-white/[0.08] rounded-2xl px-5 py-4 text-sm text-slate-200 placeholder:text-slate-600 focus:outline-none focus:border-violet-500/40 focus:bg-white/[0.05] transition-all resize-none font-sans leading-relaxed"
                    />
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full bg-white/[0.02] border border-white/[0.06] rounded-xl pl-9 pr-4 py-2.5 text-xs text-slate-400 placeholder:text-slate-600 focus:outline-none focus:border-violet-500/30 transition-all font-sans"
                    />
                  </div>
                </>
              ) : (
                <>
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] font-display">Transaction Data</label>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full bg-white/[0.03] border border-white/[0.08] rounded-2xl pl-11 pr-5 py-4 text-sm text-slate-200 placeholder:text-slate-600 focus:outline-none focus:border-cyan-500/40 focus:bg-white/[0.05] transition-all font-sans"
                        disabled={isAnalyzing}
              ...
              <div className="flex gap-3 pt-2">
                <button
                  onClick={analyze}
                  disabled={(activeTab === "message" ? !message.trim() : !amount.trim()) || isAnalyzing}
                  className={`flex-1 flex items-center justify-center gap-3 py-4 rounded-2xl text-white text-sm font-black uppercase tracking-widest transition-all active:scale-[0.98] disabled:opacity-30 disabled:cursor-not-allowed shadow-xl font-display
                    ${activeTab === "message" ? "bg-gradient-to-r from-violet-600 to-fuchsia-600 hover:from-violet-500 hover:to-fuchsia-500 shadow-violet-500/20" : "bg-gradient-to-r from-cyan-600 to-emerald-600 hover:from-cyan-500 hover:to-emerald-500 shadow-cyan-500/20"}`}
                >
                  {isAnalyzing ? (
                    <><Loader2 className="w-4 h-4 animate-spin" /> Analyzing…</>
                  ) : (
                    <><Shield className="w-4 h-4" /> Scan {activeTab === "message" ? "Text" : "Behavior"}</>
                  )}
                </button>
                {(messageResult || behaviorResult) && (
                  <button
                    onClick={reset}
          ...
          <AnimatePresence>
            {error && (
              <motion.div
                ...
                className="flex items-center gap-3 p-4 bg-red-500/10 border border-red-500/20 rounded-2xl text-sm text-red-400 font-sans"
              >
            ...
            {activeTab === "message" && messageResult && !isAnalyzing && (
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-6 rounded-3xl bg-white/[0.03] border border-white/[0.06] space-y-4">
                    ...
                    <div className="flex items-center gap-2 pt-1">
                      ...
                      <span className="text-xs text-slate-400 font-bold font-display uppercase tracking-wider">{messageResult.final_decision.scam_type} pattern</span>
                    </div>
                  </div>
                ...
                <div className="p-5 rounded-2xl bg-white/[0.02] border border-white/[0.06] flex items-start gap-4">
                  ...
                  <p className="text-sm text-slate-300 leading-relaxed font-sans">{messageResult.reasoning.summary}</p>
                </div>

                <div className="space-y-3">
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] font-display">Detected Signals</p>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    <SignalChip label="Urgency" active={hasSignal('urgenc') || hasSignal('act') || hasSignal('limited')} icon={<Zap className="w-3.5 h-3.5" />} />
                    <SignalChip label="Threat" active={hasSignal('threat') || hasSignal('block') || hasSignal('legal') || hasSignal('suspend')} icon={<AlertTriangle className="w-3.5 h-3.5" />} />
                    <SignalChip label="Payment Request" active={hasSignal('payment') || hasSignal('fee') || hasSignal('upi') || hasSignal('money')} icon={<CreditCard className="w-3.5 h-3.5" />} />
                    ...
                    <SignalChip label="Repetition" active={hasSignal('repeat') || hasSignal('repetition')} icon={<Eye className="w-3.5 h-3.5" />} />
                  </div>
                </div>
                ...
                <div className="rounded-2xl border border-white/[0.06] overflow-hidden">
                  <button onClick={() => setShowReasoning(!showReasoning)} className="w-full flex items-center justify-between px-5 py-4 bg-white/[0.02] hover:bg-white/[0.04] transition-all">
                    <span className="text-[11px] font-black text-slate-400 uppercase tracking-widest font-display">Detailed AI Reasoning</span>
          ...
          <AnimatePresence>
            {activeTab === "behavior" && behaviorResult && !isAnalyzing && (
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-6 rounded-3xl bg-white/[0.03] border border-white/[0.06] space-y-4">
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] font-display">Behavior Verdict</p>
                  ...
                  <div className="p-6 rounded-3xl bg-white/[0.03] border border-white/[0.06] flex flex-col items-center justify-center">
                    ...
                    <div className="flex items-center gap-2 mt-1 px-3 py-1 rounded-full bg-white/[0.03] border border-white/[0.05]">
                      <span className="text-[10px] font-black text-slate-500 font-display uppercase tracking-widest">{behaviorResult.risk}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-5 rounded-2xl bg-white/[0.02] border border-white/[0.06] flex items-start gap-4">
                  <Info className="w-5 h-5 text-cyan-400 mt-0.5 shrink-0" />
                  <p className="text-sm text-slate-300 leading-relaxed font-sans">{behaviorResult.reasoning.summary}</p>
                </div>

                <div className="space-y-3">
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] font-display">Behavior Signals</p>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
